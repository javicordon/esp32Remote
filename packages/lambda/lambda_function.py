...
def set_thing_auto(thingName, state):
    # Change topic, qos and payload
    client = boto3.client('iot-data', region_name='us-west-1')
    payload = json.dumps({'state': { 'desired': { 'auto': { 'onboard': state } } }})
...
@sb.request_handler(can_handle_func=is_intent_name("autoModeOn"))
def setAutoModeON_handler(handler_input):
    """Handler to turn on AC auto Mode"""

    thingName = "remotePrototype01"

    shadow = get_thing_state(thingName)
    shadowReported = shadow["state"]["reported"]
    shadowAuto = shadowReported["auto"]["onboard"]
    print("Current Auto State", shadowAuto)

    autoVal = 1
    autoValString = str(1)

    rPayload = set_thing_auto(thingName, autoValString)

    speech_text = f"Ok. AC is now in Auto Mode."

    return handler_input.response_builder.speak(speech_text).set_card(
        SimpleCard("Auto Mode AC", speech_text)).set_should_end_session(
        True).response

@sb.request_handler(can_handle_func=is_intent_name("autoModeOff"))
def setAutoModeOff_handler(handler_input):
    """Handler to turn off AC auto Mode"""
    ...
    speech_text = f"Ok. AC Auto Mode is Off."
    return handler_input.response_builder.speak(speech_text).set_card(
        SimpleCard("Auto Mode AC", speech_text)).set_should_end_session(
        True).response
