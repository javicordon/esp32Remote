...
def mqtt_subscribe(topic, msg):
    ...
    try:
        if message['state']['auto']:
            print('detected auto msg')
            global auto_mode
            auto_mode = message['state']['auto']['onboard']
            print('Auto Mode received', auto_mode)
    except:
        print('No Auto Command Detected')
    #print("Done")
...
#Using the message above, the device shadow is updated.
    try:
        ...
        print(e)
        display.fill(0)
        display.text('Error', 3, 0, 1)
        display.text(str(e),15, 8, 1)
        display.show()
        sleep(5)
        reset()
